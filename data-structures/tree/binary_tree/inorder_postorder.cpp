...
(
    ...
    intmax_t * postorder_begin,
    intmax_t * postorder_end
)
{
    ...
    node->left = create_tree(inorder_begin, inorder_begin + size, postorder_begin, postorder_begin + size);
    node->right = create_tree(inorder_begin + size + 1, inorder_end, postorder_begin + size, postorder_end - 1);
    node->data = root;
...
{
    {
        ...
        intmax_t postorder_array[7] = {3, 4, 1, 5, 6, 2, 0};

        tree_node * root = create_tree(inorder_array + 0, inorder_array + 7, postorder_array + 0, postorder_array + 7);
